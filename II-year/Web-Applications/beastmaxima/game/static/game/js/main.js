...
(() => {
  ...
  var require_main = __commonJS({
    "game/static/game/ts/main.ts"() {
      ...
      function buildBoard(boardElement = document.getElementById("board")) {
        if (!boardElement) {
        ...
        for (let row = 0; row < size + 2; row++) {
          for (let col = 0; col < size + 2; col++) {
            ...
            if (isTopRow && isLeftCol || isTopRow && isRightCol || isBottomRow && isLeftCol || isBottomRow && isRightCol) {
      ...
      function setupClueCell(cell, clueArrayName, index) {
        cell.readOnly = true;
        const clueValue = currentPuzzle.board[clueArrayName][index];
        if (clueValue !== null) {
      ...
      function setupBoardCell(cell, row, col) {
        cell.classList.add("cell", "cell-input");
        const cellValue = currentPuzzle.board.gameBoard[row][col];
        if (cellValue !== null) {
      ...
      function handleInput(row, col, cell) {
      ...
      function handleKeyDown(event, row, col, cell) {
        let targetRow = row;
        let targetCol = col;
        switch (event.key) {
          case "ArrowUp":
        ...
        const targetCell = boardElement.querySelector(`input[data-row="${targetRow}"][data-col="${targetCol}"]`);
      ...
      async function checkBoard() {
        ...
        const result = await apiFetch("POST", "/game/api/validate/", payload);
        if (result.ok) {
...
Keep trying!`);
      ...
      function showGameMessage(title, message, onCloseCallback) {
...
${message}`);
      ...
      async function fetchUpdatedRanking() {
        const result = await apiFetch("GET", "/game/api/ranking/");
        if (result.ok) {
          const leaderBoard = result.data.global_leaderboard;
      ...
      async function fetchSizeRanking(size) {
        const result = await apiFetch("GET", `/game/api/ranking/size/${size}/`);
        if (result.ok) {
      ...
      document.addEventListener(
        ...
        () => {
          ...
          if (boardElement) {
            ...
            setInterval(fetchUpdatedRanking, 15e3);
          }
        }
      );
    }
