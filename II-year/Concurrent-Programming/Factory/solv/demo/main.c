...
int test1()
{
  ...
  printf("Waiting for task id %d to complete...\n", t.id);

  int task_result = collect_task(&t);

  printf("Result collected");

  if (task_result == PLANTOK) {
    printf("Task id %d collected\n", t.id);
    printf("Collected %d %d\n", t.results[0], t.results[1]);

  } else if (task_result == ERROR) {
    printf("Error collecting task id %d\n", t.id);
    printf("Results may be incomplete: %d %d\n", t.results[0], t.results[1]);
  }
